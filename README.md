...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            .print-grid div { margin-bottom: 4px; }
            ...
            .print-guidelines h4 { margin: 0 0 5px 0; font-size: 12px; color: #1f2937; }
            .print-guidelines ul { margin: 0; padding-left: 20px; color: #374151;}
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="card rounded-xl p-5 shadow-sm">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium mb-1">Objetivo (Gera recomendação)</label>
                ...
                <div class="card rounded-xl p-5 shadow-sm">
                    ...
                    <p class="text-xs opacity-70 mb-3">Selecione para gerar diretrizes automáticas na ficha impressa.</p>
                    <div class="grid grid-cols-1 gap-2 text-sm" id="health-container">
                        <!-- Gerado via JS -->
                    </div>
                </div>
    ...
    <script>
        ...
        const dbCategories = {
            ...
            "💪 BRAÇOS": [
                ...
                "(Tríceps) Arremesso", "(Tríceps) Testa", "(Tríceps) Mergulho no Banco"
            ],
            "🪨 OMBROS": [
        ...
        function renderHealthOptions() {
            ...
            container.innerHTML = Object.keys(healthData).map(opt => `
                <label class="flex items-start space-x-3 cursor-pointer p-2 rounded hover:bg-black hover:bg-opacity-5 transition border border-transparent hover:border-gray-500 hover:border-opacity-20">
                    <input type="checkbox" value="${opt}" class="health-cb mt-1 rounded border-gray-400 text-primary focus:ring-primary w-4 h-4">
                    ...
                    exercisesHtml = workout.exercises.map((ex, idx) => `
                        <div class="flex flex-col xl:flex-row gap-3 p-3 items-start xl:items-center border-b last:border-0 border-opacity-10" style="border-color: var(--border-color)">
                            ...
                            <div class="flex flex-wrap gap-2 w-full xl:w-auto">
                                ...
                                <select onchange="updateExercise('${workout.id}', ${idx}, 'technique', this.value)" class="input-field w-32 rounded px-2 py-1 text-xs">
        ...
        function renderModalCategories() {
            ...
            container.innerHTML = Object.keys(dbCategories).map(cat => `
                <button onclick="setModalCategory('${cat}')" class="w-full text-left px-3 py-3 rounded-lg text-sm font-medium transition ${state.activeCategory === cat ? 'bg-primary text-white shadow-md' : 'hover:bg-black hover:bg-opacity-10'}">
                    ${cat}
                </button>
            `).join('');
        }
            ...
            if(data.recs.trim()) {
                html += `
                    <div class="print-recommendations">
                        <h4 style="margin-top:0; margin-bottom: 5px;">Recomendações Manuais do Profissional</h4>
